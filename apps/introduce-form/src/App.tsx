...
const IntroduceForm: React.FC = () => {
  ...
  const [showBackgroundResizeModal, setShowBackgroundResizeModal] = useState(false);
  ...
  const [backgroundImageRef, setBackgroundImageRef] = useState<HTMLImageElement | null>(null);
  ...
  const seriesNames = {
    ...
    영어: {
      ...
      newWorks: [
        '06 EoSD', '07 PCB', '08 IN', '09 PoFV', '10 MoF',
        '11 SA', '12 UFO', '13 TD', '14 DDC', '15 LoLK',
        '16 HSiFS', '17 WBaWC', '18 UM', '19 WBaWC', '20 FW',
      ],
    },
  };
  ...
  const uiTexts = {
    한국어: {
      ...
      dislikedContentDetailPlaceholder: '불호하는 소재나 지뢰를 자세히 적어주세요',
    },
    일본어: {
      ...
      dislikedContentDetailPlaceholder: '苦手な題材や地雷を詳しく書いてください',
    },
    영어: {
      ...
      dislikedContentDetailPlaceholder: 'Please describe your disliked content or triggers in detail',
    },
  };

  const currentTexts = uiTexts[formData.language as keyof typeof uiTexts] || uiTexts.한국어;
  ...
  const handleMainSeriesChange = (type: 'oldWorks' | 'newWorks', value: string) => {
    setFormData((prev) => ({
      ...prev,
      mainSeries: {
        ...prev.mainSeries,
  ...
  const handleToggleAllAccountTypes = () => {
    const accountTypeKeys = [
      'writing', 'drawing', 'gaming', 'story', 'consumption', 'subscription',
      'cosplay', 'retweet', 'likes', 'daily', 'timeline', 'fangirling',
    ] as const;
    const currentSelected = accountTypeKeys.filter((key) => formData.accountType[key]);
    const allSelected = currentSelected.length === accountTypeKeys.length;
    setFormData((prev) => ({
      ...prev,
      accountType: {
        ...prev.accountType,
        ...Object.fromEntries(accountTypeKeys.map((key) => [key, !allSelected])),
      },
    }));
  };
  ...
  const handleBackgroundCropComplete = async () => {
    if (backgroundImageRef && backgroundCrop.width && backgroundCrop.height) {
      try {
        const croppedImageUrl = await getBackgroundCroppedImg(backgroundImageRef, backgroundCrop, backgroundOpacity);
        setFormData((prev) => ({ ...prev, backgroundImage: croppedImageUrl }));
  ...
  const handleTextAreaResize = (field: keyof FormData['textAreaHeights'], height: number) => {
    setFormData((prev) => ({
      ...prev,
      textAreaHeights: { ...prev.textAreaHeights, [field]: height },
    }));
  };
  ...
  const generateImage = async () => {
    ...
    try {
      ...
      const dynamicHeight = Math.max(600, scrollHeight);
      ...
      if (formData.backgroundImage) {
        ...
        const canvas = await html2canvas(imageFormRef.current, {
          ...
          scrollX: 0, scrollY: 0,
          windowWidth: 800, windowHeight: dynamicHeight,
        });
    ...
    <div className={styles.container}>
      <div className={styles.formContainer}>
            ...
            <div className={styles.profileImageSection}>
              <div className={styles.profileImageBox}>
                {formData.profileImage ? (
                  <img src={formData.profileImage} alt="프로필 이미지" className={styles.profileImage} />
                ) : (
                ...
                {currentTexts.fubOptions.map((option) => (
                  <label key={option} className={styles.checkboxLabel}>
                    <input
                      ...
                      onChange={(e) => handleInputChange('fubFree', e.target.value)}
                    />
                    {option}
                  </label>
                ))}
                ...
                {currentTexts.partingOptions.map((option) => (
                  <label key={option} className={styles.checkboxLabel}>
                    <input
                      ...
                      onChange={(e) => handleInputChange('parting', e.target.value)}
                    />
                    {option}
                  </label>
                ))}
                ...
                {currentTexts.genreOptions.map((option) => (
                  <label key={option} className={styles.checkboxLabel}>
                    <input
                      ...
                      onChange={(e) => handleInputChange('otherGenres', e.target.value)}
                    />
                    {option}
                  </label>
                ))}
          ...
          <div className={styles.middleColumn}>
            <div className={styles.formSection}>
                  ...
                  {oldWorksOptions.map((option) => (
                    <label key={option} className={styles.checkboxLabel}>
                      <input
                        ...
                        onChange={() => handleMainSeriesChange('oldWorks', option)}
                      />
                      {option}
                    </label>
                  ))}
                  ...
                  {newWorksOptions.map((option) => (
                    <label key={option} className={styles.checkboxLabel}>
                      <input
                        ...
                        onChange={() => handleMainSeriesChange('newWorks', option)}
                      />
                      {option}
                    </label>
                  ))}
              ...
              <div className={styles.seriesSection}>
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleInputChange('mainSeries', { ...formData.mainSeries, others: e.target.value })
                  }
                  className={styles.textInput}
                />
              </div>
            ...
            <div className={styles.formSection}>
              ...
              <div className={styles.accountTypeGrid}>
                {[
                  ...
                  <label key={item.key} className={styles.checkboxLabel}>
                    <input
                      type="checkbox"
                      checked={formData.accountType[item.key as keyof FormData['accountType']] as boolean}
                      onChange={(e) =>
                        handleAccountTypeChange(item.key as keyof FormData['accountType'], e.target.checked)
                      }
                    />
                    {item.label}
                  </label>
              ...
              <div className={styles.accountTypeExtra}>
                <input
                  ...
                  onChange={(e) => handleAccountTypeChange('other', e.target.value)}
                  className={styles.textInput}
                />
              </div>
            </div>
            ...
            <div className={styles.formSection}>
                ...
                {currentTexts.dislikedContentOptions.map((option) => (
                  <label key={option} className={styles.checkboxLabel}>
                    <input
                      ...
                      onChange={(e) => handleInputChange('dislikedContent', e.target.value)}
                    />
                    {option}
                  </label>
                ))}
              ...
              {formData.dislikedContent === currentTexts.exists && (
                <div className={styles.formSection} style={{ marginTop: '15px' }}>
                  ...
                  <textarea
                    ...
                    maxLength={100}
                    style={{
                      height: Math.min(formData.textAreaHeights.dislikedContentDetail, 200),
                      maxHeight: '200px',
                      overflowY: 'auto',
                    }}
                    onMouseUp={(e) => {
                      const target = e.target as HTMLTextAreaElement;
                      handleTextAreaResize('dislikedContentDetail', Math.min(target.scrollHeight, 200));
                    }}
                    placeholder={currentTexts.dislikedContentDetailPlaceholder}
                  />
                  <div className={styles.charCount}>{formData.dislikedContentDetail.length}/100</div>
                </div>
              )}
            </div>
          ...
          <div className={styles.rightColumn}>
            <div className={styles.formSection}>
              ...
              <textarea
                ...
                maxLength={100}
                style={{
                  height: Math.min(formData.textAreaHeights.favoriteCharacter, 200),
                  maxHeight: '200px',
                  overflowY: 'auto',
                }}
                onMouseUp={(e) => {
                  const target = e.target as HTMLTextAreaElement;
                  handleTextAreaResize('favoriteCharacter', Math.min(target.scrollHeight, 200));
                }}
                placeholder={`${currentTexts.favoriteCharacter} (최대 100자)`}
              />
              <div className={styles.charCount}>{formData.favoriteCharacter.length}/100</div>
            </div>
            ...
            <div className={styles.formSection}>
              ...
              <textarea
                ...
                onMouseUp={(e) => {
                  const target = e.target as HTMLTextAreaElement;
                  handleTextAreaResize('pairing', Math.min(target.scrollHeight, 200));
                }}
                placeholder={`${currentTexts.pairing} (최대 100자)`}
              />
              <div className={styles.charCount}>{formData.pairing.length}/100</div>
            </div>
            ...
            <div className={styles.formSection}>
              ...
              <textarea
                ...
                maxLength={300}
                style={{
                  height: Math.min(formData.textAreaHeights.freeDescription, 300),
                  maxHeight: '300px',
                  overflowY: 'auto',
                }}
                onMouseUp={(e) => {
                  const target = e.target as HTMLTextAreaElement;
                  handleTextAreaResize('freeDescription', Math.min(target.scrollHeight, 300));
                }}
                placeholder={`${currentTexts.freeDescription} (최대 300자)`}
              />
              <div className={styles.charCount}>{formData.freeDescription.length}/300</div>
            </div>
          </div>
        </div>
        ...
        <div className={styles.backgroundImageSection}>
          ...
          <input type="file" accept="image/*" onChange={handleBackgroundImageUpload} className={styles.fileInput} />
        </div>
        ...
        <div className={styles.dataButtonContainer}>
          ...
          <button onClick={() => importInputRef.current?.click()} className={styles.dataButton}>
            📥 JSON 불러오기
          </button>
          <input
            ref={importInputRef}
        ...
        >
          ...
          <div className={styles.imageGrid}>
            <div className={styles.imageLeftColumn}>
              <div className={styles.imageProfileImage} style={{ alignSelf: 'center' }}>
                {formData.profileImage && (
                  <img
                    src={formData.profileImage}
                    alt="프로필 이미지"
                    style={{ width: '100%', height: '100%', objectFit: 'contain', backgroundColor: '#f0f0f0', maxWidth: '100%', maxHeight: '100%' }}
                  />
                )}
              </div>
            ...
            <div className={styles.imageMiddleColumn}>
              <div className={styles.imageField}>
                  ...
                  <div className={styles.imageCheckboxGrid}>
                    {oldWorksOptions.map((option) => (
                      <span key={option} className={styles.imageCheckbox}>
                        {formData.mainSeries.oldWorks.includes(option) ? '■' : '□'} {option}
                      </span>
                    ))}
                  </div>
                </div>
                <div>
                  ...
                  <div className={styles.imageCheckboxGrid}>
                    {newWorksOptions.map((option) => (
                      <span key={option} className={styles.imageCheckbox}>
                        {formData.mainSeries.newWorks.includes(option) ? '■' : '□'} {option}
                      </span>
                    ))}
                  </div>
                </div>
                <div>
                  <h4>{currentTexts.others}:</h4>
                  <div className={styles.imageTextValue}>{formData.mainSeries.others || currentTexts.none}</div>
                </div>
              </div>
              ...
              <div className={styles.imageField}>
                ...
                <div className={styles.imageCheckboxGrid}>
                  {[
                    ...
                    <span key={item.key} className={styles.imageCheckbox}>
                      {(formData.accountType[item.key as keyof FormData['accountType']] as boolean) ? '■' : '□'} {item.label}
                    </span>
                  ))}
                </div>
                {formData.accountType.other && (
                  <div>
                    ...
                    <div className={styles.imageTextValue}>{formData.accountType.other}</div>
                  </div>
                )}
              </div>
              ...
              <div className={styles.imageField}>
                ...
                {formData.dislikedContent === currentTexts.exists && formData.dislikedContentDetail && (
                  <div>
                    <h4>{currentTexts.dislikedContentDetail}:</h4>
                    <div className={styles.imageTextValue}>
                      {formData.dislikedContentDetail.split('\n').map((line, index) => (
            ...
            <div className={styles.imageRightColumn}>
              ...
              <div className={styles.imageField}>
                ...
                <div
                  ...
                  style={{ minHeight: formData.textAreaHeights.freeDescription }}
                >
        ...
        <div className={styles.modalOverlay}>
          <div className={styles.modalContent}>
              ...
              >
                <img
                  ...
                  onLoad={(e) => {
                    ...
                    setCrop({ unit: 'px', width: size, height: size, x: (imgWidth - size) / 2, y: (imgHeight - size) / 2 });
                  }}
            ...
            <div className={styles.modalButtons}>
              <button onClick={handleCancelCrop} className={styles.cancelButton}>취소</button>
              <button onClick={handleCropComplete} className={styles.saveButton}>저장</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showBackgroundResizeModal && tempBackgroundImage && (
        <div className={styles.modalOverlay}>
          <div className={styles.modalContent}>
              ...
              >
                <img
                  ...
                  onLoad={(e) => {
                    ...
                    setBackgroundCrop({ unit: 'px', width: cropWidth, height: cropHeight, x, y });
                  }}
                  style={{ maxWidth: '100%', maxHeight: '400px' }}
                />
              </ReactCrop>
            ...
            <div className={styles.modalButtons}>
              <button onClick={handleCancelBackgroundCrop} className={styles.cancelButton}>취소</button>
              <button onClick={handleBackgroundCropComplete} className={styles.saveButton}>저장</button>
            </div>
          </div>
        </div>
      )}
